...
- Hard Triplet loss, Identity Softmax loss, Center loss를 통해 모델을 학습한다.
- Baseline과 달리 feature embedding에 batch normalization을 사용하지 않았다.
- Baseline과 달리 triplet loss의 variation인 Soft Margin loss를 사용하였다.
- 최고 성능: unknown
"""
...
class Triarchy(nn.Module):

    def __init__(self, args, num_classes, train=False):
        """train=False로 지정할 경우 inference에 사용되지 않는 layer는 생성하지 않음"""
        super().__init__()
        self.args = args
        ...
        # Center loss를 위한 center feature들
        self.center_features = nn.Linear(512, self.num_classes, bias=False) if train else None
    
    def forward(self, x, train):
        x = self.backbone(x)
        
        feature = self.fc1(x)  # feature embedding
        del x
        if not train:
           return feature
    ...
    def get_cost(self, data):
        ...
        # triplet loss
        triplet_cost = TripletHardLoss()(features, pid_indices.cuda())

        # center loss
        center_features = self.center_features.weight[pid_indices]  # torch.Size([batch_size, 512])
        center_cost = torch.mean(torch.sum((features - center_features)**2, dim=1))
        del features
        
        cost = identity_cost + triplet_cost + center_cost
        return cost, [identity_cost.item(), triplet_cost.item(), center_cost.item()]
